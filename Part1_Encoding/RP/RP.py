...
def encode_dataset(dataset,batch_size,downscale_factor,pooling_function,dimension=1,time_delay=1,threshold=None):
    ...
    rp = RecurrencePlot(dimension=dimension, time_delay=time_delay,threshold=threshold)
    
    print('Encoding started...')
    for p in range(n_batches):
        if p==0:
            X_rp = rp.transform(dataset[0:batches[p],:])
            sample=block_reduce(X_rp[0], block_size=(f, f), func=pooling_function)
            ...
            for i in range(0,batches[p]):
                X_rp_red[i] = block_reduce(X_rp[j], block_size=(f, f) , func=pooling_function)
                j+=1
...
if __name__=='__main__':
    ...
    X_rp=encode_dataset(dataset=X,batch_size=172,downscale_factor=8, pooling_function=np.mean,dimension=1,time_delay=1,threshold=None)
    X_test_rp=encode_dataset(dataset=X_test,batch_size=60,downscale_factor=8, pooling_function=np.mean,dimension=1,time_delay=1,threshold=None)
    
    save_data(directory='RP_Images',dataset_name='X_rp.npy',dataset=X_rp)
    save_data(directory='RP_Images',dataset_name='X_test_rp.npy',dataset=X_test_rp)


# Display Images
##############################################
X_rp=np.load('RP_Images/X_rp.npy')
sample_image=X_rp[0] 
